...
test.describe('Sheet states', () => {
  ...
  test('filter modal has "Alleen bewaard" option', async ({ page, isMobile }) => {
    if (!isMobile) test.skip();
    await page.goto('/app.html');
    await page.locator('#map').waitFor({ state: 'visible', timeout: 15000 });
...
test.describe('Accessibility', () => {
  ...
  test('filter chips have correct ARIA attributes', async ({ page, isMobile }) => {
    if (!isMobile) test.skip();
    await page.goto('/app.html');
    await page.locator('#map').waitFor({ state: 'visible', timeout: 15000 });

    const chips = page.locator('#sheet-filter-chips');
    await expect(chips).toHaveAttribute('role', 'toolbar');

    const filterChips = page.locator('.sheet-filter-chip');
    const count = await filterChips.count();
    expect(count).toBeGreaterThanOrEqual(5);

    // Each chip should have aria-pressed
    for (let i = 0; i < count; i++) {
      await expect(filterChips.nth(i)).toHaveAttribute('aria-pressed', /(true|false)/);
    }
  });
