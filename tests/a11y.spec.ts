...
test.describe('Accessibility', () => {
  ...
  test('filter chips have proper ARIA toolbar role', async ({ page, isMobile }) => {
    if (!isMobile) test.skip();

    await page.goto('/app.html');
    await expect(page.locator('#map')).toBeVisible({ timeout: 15000 });

    // Filter chips toolbar should have role="toolbar"
    const toolbar = page.locator('#sheet-filter-chips');
    await expect(toolbar).toHaveAttribute('role', 'toolbar');

    // Each chip should be a button
    const chips = page.locator('.sheet-filter-chip');
