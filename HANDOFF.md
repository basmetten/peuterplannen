...
**Pre-production bugfix sprint complete (full)** — All 14 critical + 15 high-priority fixes from BUGFIX-PRE-PHASE9.md applied. Session 1 (C1-C7, H1-H13): font CSS vars, Silk handle selector, activeDetent fallback, map error resilience, flyTo sequencing, SheetErrorBoundary, panTimer leak, inert panels, hydration-safe localStorage, 44px tap targets, LocationCard dedup, canonical share URL, map.resize on collapse, scroll-to-drag, Silk cleanup, detail skeleton, weather dedup, Silk pinned, accent color sync. Session 2 (C8-C14, H14-H15): peek state only shows search bar (snap gate), guide→location closes guide first, desktop guide back button, StackedSheet close 44px, "Bekijk alle" 44px, shared GuideCard component, emoji→SVG, E2E tests updated for Phase 9 architecture, guide recent list date-sorted.
**Post-bugfix QA fixes** — Guide scroll blocked (scrollGestureTrap disabled when swipe=false), NEXT_PUBLIC_R2_URL added to staging build env (images were blank), CategoryGrid inline gridTemplateColumns fallback (Silk CSS layer override), desktop snap='full' override (peek gate was hiding sidebar content), E2E tests rewritten with desktop-aware expandSheet, force:true for Silk overlay clicks, staging test config added.
