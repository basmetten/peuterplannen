...
**Pre-production bugfix sprint complete** — 7 critical + 13 high-priority fixes from BUGFIX-PRE-PHASE9.md. Criticals: font CSS vars fixed (Inter/Newsreader), Silk handle selector corrected (a17), activeDetent ?? fallback, map error handler resilience, flyTo sequenced after sheet animation, SheetErrorBoundary added, panTimer leak fixed. High: inert on off-screen panels, hydration-safe localStorage, 44px tap targets (heart + filter pills), LocationCard double-click fix, canonical share URL, map.resize on sidebar close, StackedSheet scroll-to-drag, Silk props cleanup, detail skeleton layout shift fix, weather deduplication, Silk pinned to 0.10.1, accent color doc sync.
