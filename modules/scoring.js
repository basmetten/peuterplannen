...
/**
 * Check if a location is currently open based on always_open flag or opening_hours text.
 * Parses common Dutch opening hours formats like "Ma-Vr 09:00-17:00, Za 10:00-16:00".
 * @param {Object} loc - Location object
 * @returns {boolean}
 */
export function isLocationOpenNow(loc) {
    if (loc.always_open) return true;
    ...
    // Normalize: lowercase, split on comma or semicolon
    const segments = loc.opening_hours.toLowerCase().replace(/\s+/g, ' ').split(/[,;]+/).map(s => s.trim());

    for (const segment of segments) {
        // Match patterns like "ma-vr 09:00-17:00" or "za 10:00-16:00" or "dag 08:00-18:00"
        const match = segment.match(/^([\w\-–]+)\s+(\d{1,2})[:\.](\d{2})\s*[-–]\s*(\d{1,2})[:\.](\d{2})/);
        if (!match) continue;

        const dayPart = match[1];
        const openMinutes = parseInt(match[2]) * 60 + parseInt(match[3]);
        const closeMinutes = parseInt(match[4]) * 60 + parseInt(match[5]);

        // "dag" or "daily" means every day
        if (dayPart === 'dag' || dayPart === 'daily') {
            if (currentMinutes >= openMinutes && currentMinutes <= closeMinutes) return true;
            continue;
        }

        // Check if dayPart is a range (ma-vr) or single day (za)
        const rangeParts = dayPart.split(/[-–]/);
        let days = [];
        if (rangeParts.length === 2 && dayMap[rangeParts[0]] !== undefined && dayMap[rangeParts[1]] !== undefined) {
            const start = dayMap[rangeParts[0]];
            const end = dayMap[rangeParts[1]];
            // Build day range (handles wrap-around, e.g. vr-ma)
            let d = start;
            while (true) {
                days.push(d);
                if (d === end) break;
        ...
        if (days.includes(dayIndex) && currentMinutes >= openMinutes && currentMinutes <= closeMinutes) {
            return true;
        }
    }
...
export function getDecisionHeadline(item, travelInfo) {
    ...
    if (state.activePreset === 'now-open') return 'Nu open — direct naartoe';
...
export function getPracticalBullets(item) {
    ...
    const simpleLabels = {
        ...
        confident: 'goed', none: null, unknown: null, varies: 'wisselend'
    };
    if (item.time_of_day_fit && timeLabels[item.time_of_day_fit]) bullets.push(timeLabels[item.time_of_day_fit]);
    const _lbl = (field, label) => { const v = simpleLabels[field]; return v ? `${label}: ${v}.` : null; };
    if (item.rain_backup_quality) { const b = _lbl(item.rain_backup_quality, 'Slechtweeroptie'); if (b) bullets.push(b); }
    if (item.buggy_friendliness) { const b = _lbl(item.buggy_friendliness, 'Buggyvriendelijk'); if (b) bullets.push(b); }
    if (item.parking_ease) { const b = _lbl(item.parking_ease, 'Parkeren'); if (b) bullets.push(b); }
    if (item.food_fit) { const b = _lbl(item.food_fit, 'Eten combineren'); if (b) bullets.push(b); }
    if (item.play_corner_quality) { const b = _lbl(item.play_corner_quality, 'Speelwaarde'); if (b) bullets.push(b); }
    if (item.toilet_confidence) { const b = _lbl(item.toilet_confidence, 'Sanitair'); if (b) bullets.push(b); }
    if (item.noise_level) { const b = _lbl(item.noise_level, 'Geluidsniveau'); if (b) bullets.push(b); }
