...
/**
 * Renders a scan card with progressive disclosure:
 * 1. Photo (full-width, 5:2, type badge overlay)
...
function renderScanCard(item, travelInfo, batchIdx) {
    ...
    card.innerHTML = `
        <div class="scan-photo${!photoSrc ? ' scan-photo--category' : ''}" style="--photo-color: ${photoColor}">
            <img src="${escapeHtml(imgSrc)}" alt="${escapeHtml(item.name)}" loading="lazy" decoding="async" width="400" height="160"
