...
export function trackEvent(type, data) {
    // Respect user consent — only track if analytics_storage is granted
    try {
        const consent = localStorage.getItem('pp_consent');
        if (consent) {
            const parsed = JSON.parse(consent);
            if (parsed.analytics_storage !== 'granted') return;
        } else {
            return; // No consent given yet — don't track
        }
    } catch { return; }
